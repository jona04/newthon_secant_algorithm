...
void newton_fix(double phi, cqk_problem *restrict p, double *restrict x, 
    ...
    // printf("Fixed %d variables (from %d to %d).\n",*n - len, *n, len);
    *n = len;
}
 ...
 *     problem.
 */
double newton(cqk_problem *restrict p, double *x0, double *x) {
    ...
    while (phi != 0.0 && n_iters <= MAXITERS) {
        ...
        if (deriv != 0.0) {
            ...
            printf("New lambda breakpoint: %e\n", lambda);
        ...
        newton_phi(p, n, ind, r, slopes, lambda, x, &phi, &deriv);
        // printf("Iter %d - lambda = %e, phi - r = %e\n", 
                    // n_iters, lambda, phi);
