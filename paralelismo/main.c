...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0;i<repeticao;i++){
        ...
        // }else{
            ...
            duracao += end_omp-start_omp;
            // end1 = clock();
            // cpu_time_used1 = ((double) (end1 - start1)) / CLOCKS_PER_SEC;
            // duracao += cpu_time_used1;

            if (end_omp-start_omp > max){
                max = end_omp-start_omp;

            }
            if (end_omp-start_omp < min){
                min = end_omp-start_omp;
            }
            if (status > 0)
                count_status = count_status + status;
            else
                repeticao--;
    ...
    printf("min time %f \n",min);
    printf("max time %f \n",max);
    printf("media iteracoes %f\n",count_status/repeticao);
